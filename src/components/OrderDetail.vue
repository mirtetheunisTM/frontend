...
onMounted(async () => {
  ...
  try {
    ...
    const response = await fetch(`https://backend-lc9k.onrender.com/api/v1/orders/${route.params.id}` /*`http://localhost:3000/api/v1/orders/${route.params.id}`*/, {
      'headers': {
 ...
 async function updateOrderStatus() {
  try {
    const response = await fetch(`https://backend-lc9k.onrender.com/api/v1/orders/${order.value._id}` /*`http://localhost:3000/api/v1/orders/${order.value._id}`*/, {
      method: 'PUT',
      headers: { 
  ...
  <div class="bg-black min-h-screen flex justify-center items-start py-10 text-white">
    ...
    <div v-if="showConfirm" class="fixed inset-0 flex justify-center items-center backdrop-blur-sm bg-customGray bg-opacity-50">
      <div class="bg-gray-700 py-5 px-10 rounded-3xl">
        <p class="text-lg mb-8 text-white">Are you sure you want to {{ currentAction }} this order?</p>
